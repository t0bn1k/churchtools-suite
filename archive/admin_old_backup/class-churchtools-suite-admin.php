...
class ChurchTools_Suite_Admin {
	...
	public function ajax_run_update() {
		<?php
		// Backwards-compatible shim: admin files moved to /admin/
		// This file kept to avoid fatal require errors from older installs.

		if ( ! defined( 'ABSPATH' ) ) {
			exit;
		}

		// Load the new admin class location
		$new_admin = CHURCHTOOLS_SUITE_PATH . 'admin/class-churchtools-suite-admin.php';
		if ( file_exists( $new_admin ) ) {
			require_once $new_admin;
		} else {
			// If the new admin file is missing, trigger a clear error to help debugging
			trigger_error( 'Missing admin class file: ' . $new_admin, E_USER_WARNING );
		}
			// Load dependencies
	...
	public function ajax_reload_logs() {
		...
		try {
			...
			if ( empty( $log_content ) ) {
				$html = '<span class="cts-muted">Keine Logs verfügbar. Führen Sie einen Sync aus, um Logs zu generieren.</span>';
			} else {
				// Colorize log levels using CSS classes
				$log_content = htmlspecialchars( $log_content );
				$log_content = preg_replace( '/\[ERROR\]/', '<span class="cts-error cts-log-level">[ERROR]</span>', $log_content );
				$log_content = preg_replace( '/\[WARNING\]/', '<span class="cts-warning cts-log-level">[WARNING]</span>', $log_content );
				$log_content = preg_replace( '/\[INFO\]/', '<span class="cts-info-box cts-log-level">[INFO]</span>', $log_content );
				$html = $log_content;
			}
	...
	public function ajax_clear_logs() {
		...
		try {
			...
			wp_send_json_success( [
				...
				'html' => '<span class="cts-muted">Logs wurden gelöscht. Führen Sie einen Sync aus, um neue Logs zu generieren.</span>'
			] );
